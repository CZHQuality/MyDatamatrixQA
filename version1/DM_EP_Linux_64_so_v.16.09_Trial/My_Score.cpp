	 ...
	 result = pDecodeDM_Bits(dec1, rowcount, colcount, pbits);
         ...
         if(dmq>0 && rr == DM_RR_OK)
         {
           ...
           for (i=0;i<=dmq-1;i++){
             ...
             printf("\n");
             */
             P_Groth = fabs(pdm_info->quality.vertical_print_growth);
             if(fabs(pdm_info->quality.horizontal_print_growth) > P_Groth)
             {
                 P_Groth = fabs(pdm_info->quality.horizontal_print_growth);
             }
             if(P_Groth>=0 && P_Groth<0.10)
             {
                 P_Groth_Grade = 4;
             }
             if(P_Groth>=0.10 && P_Groth<0.50)
             {
                 P_Groth_Grade = 3;
             }
             if(P_Groth>=0.50 && P_Groth<0.90)
             {
                 P_Groth_Grade = 2;
             }
             if(P_Groth>=0.90 && P_Groth<1.30)
             {
                 P_Groth_Grade = 1;
             }
             if(P_Groth>=1.30)
             ...
             Final_Score = (float)(pdm_info->quality.axial_nonuniformity_grade) + (float)(pdm_info->quality.grid_nonuniformity_grade) + (float)(pdm_info->quality.symbol_contrast_grade) + (float)(pdm_info->quality.unused_error_correction_grade) +  (float)(pdm_info->quality.fixed_pattern_damage_grade) + (float)(pdm_info->quality.modulation_grade) + (float)(pdm_info->quality.decode_grade) + (float)(P_Groth_Grade);
             Final_Score = Final_Score/8;
             //printf("Print Groth = %4f\n", (float)(P_Groth));
           ...
           pdm_info = pGetDM_Info(dec1, 0);
           {
             ...
             */
             ...
             else if(P_Groth>=0.10 && P_Groth<0.50)
             {
                 P_Groth_Grade = 3;
             }
             else if(P_Groth>=0.50 && P_Groth<0.90)
             {
                 P_Groth_Grade = 2;
             }
             else if(P_Groth>=0.90 && P_Groth<1.30)
             {
                 P_Groth_Grade = 1;
             }
             else if(P_Groth>=1.30)
             {
                 P_Groth_Grade = 0;
             }
             Final_Score = (float)(pdm_info->quality.axial_nonuniformity_grade) + (float)(pdm_info->quality.grid_nonuniformity_grade) + (float)(pdm_info->quality.symbol_contrast_grade) + (float)(pdm_info->quality.unused_error_correction_grade) +  (float)(pdm_info->quality.fixed_pattern_damage_grade) + (float)(pdm_info->quality.modulation_grade) + (float)(pdm_info->quality.decode_grade) + (float)(P_Groth_Grade);
